...
// ================== MIRROR Token Cross-Chain Config ==================
// Monad is the canonical chain (tokens locked in OFTAdapter)
// Solana has LayerZero OFT (mints/burns wrapped MIRROR)

// Monad Mainnet - MirrorOFTAdapter
const monadContract: OmniPointHardhat = {
    eid: 30390, // MONAD_V2_MAINNET
    contractName: 'MirrorOFTAdapter',
}

// Solana Mainnet - OFT Store
...
export default async function () {
    ...
    const connections = await generateConnectionsConfig([
        [
            monadContract, // Monad - canonical chain with OFTAdapter
            solanaContract, // Solana - OFT (mint/burn)
            [['LayerZero Labs'], []], // [ requiredDVN[], [ optionalDVN[], threshold ] ]
            [15, 32], // [Monad to Solana confirmations, Solana to Monad confirmations]
            [SOLANA_ENFORCED_OPTIONS, EVM_ENFORCED_OPTIONS], // Solana enforcedOptions, Monad enforcedOptions
        ],
    ])

    return {
        contracts: [{ contract: monadContract }, { contract: solanaContract }],
        connections,
    }
}
